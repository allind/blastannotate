...
echo "polish 1"
bwa index $ori
bwa mem -t $threads -a $ori $fwd > aln_r1_1.sam
bwa mem -t $threads -a $ori $rev > aln_r2_1.sam
...
bwa mem -t $threads -a $oriname"_poly1.fasta"  $fwd > aln_r1_2.sam
bwa mem -t $threads -a $oriname"_poly1.fasta" $rev > aln_r2_2.sam
...
bwa mem -t $threads -a $oriname"_poly2.fasta"   $fwd > aln_r1_3.sam
bwa mem -t $threads -a $oriname"_poly2.fasta"  $rev > aln_r2_3.sam
